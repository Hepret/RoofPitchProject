...
public class StructureCalculationScheme
{
    ...
    private Dictionary<string, Point> GetPoints()
    {
        ...
        var y3 = points["2"].Y - K;
        points.Add("3", new Point(0, y3));
        
        var y4 = points["3"].Y - 1;
        points.Add("4", new Point(0, y4) );
        
        var y5 = points["4"].Y - K;
        points.Add("5", new Point(0, y5));

        var x6 = -3.5 * RY / (2 * N);
        var y6 = x6 * N + RY;
        points.Add("6", new Point(x6, y6));

        var x7A = -0.7 * (RY - K - 2 * y6) / N;
        var y7A = x7A * N + RY - K;
        points.Add("7а", new Point(x7A,y7A));

        var x7B = x7A;
        var y7B = -y7A;
        points.Add("7б", new Point(x7B, y7B));

        var x8 = x6;
        var y8 = -y6;
        points.Add("8", new Point(x8,y8));
